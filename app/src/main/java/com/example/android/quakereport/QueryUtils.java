...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            json.getJSONObject(SAMPLE_JSON_RESPONSE);
            JSONArray jsonArray = json.getJSONArray("features");
            for(int i = 0; i < jsonArray.length(); i ++){
                JSONObject earthQuakeData = jsonArray.getJSONObject(i).getJSONObject("properties");
earthquakes.add(new Earthquake(earthQuakeData.get("mag").toString(), earthQuakeData.getDouble("place"), earthQuakeData.get("time").toString()));
            }
